...
//   // Calculate average glucose only if there is data
...
//       {
//         data: [
//           checkups[checkups.length - 1]?.urinalysis || 0,
//           checkups[checkups.length - 1]?.hemoglobin || 0,
//           checkups[checkups.length - 1]?.glucose || 0,
//         ],
...
//           <Line
//             key={`line-${checkups.length}`} // Adding a key prop to force re-render
//             data={{
...
//           <Bar
//             key={`bar-${eyeScreenings.length}`} // Adding a key prop to force re-render
//             data={{
...
import { useSelector, useDispatch } from 'react-redux';
import { Line, Bar, Pie, ChartProps } from 'react-chartjs-2'; // Import Chart type
import { IoChevronBack, IoChevronForward } from 'react-icons/io5';
import { AppDispatch } from '../store';
import { GetAllCheckups, selectCheckups } from '../slice/checkupsSlice';
import { GetAllEyeScreenings, selectEyeScreenings } from '../slice/eyescreeningSlice';
...
import breadImage from '../assets/bread.jpg';
import avocadoImage from '../assets/avocado.jpg';
import quinoaImage from '../assets/quinoa.jpg'; // Fix typo in image import
import lentilsImage from '../assets/lentils.jpg';
import potatoImage from '../assets/potato.jpg';
...
const Insights: React.FC = () => {
  ...
  const checkups = useSelector(selectCheckups);
  const eyeScreenings = useSelector(selectEyeScreenings);
  const pieChartRef = useRef<ChartProps | null>(null); // Properly typed useRef
  ...
  useEffect(() => {
    // Destroy pie chart instance before re-rendering
